<template>
  <div class="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 to-blue-100">
    <div class="bg-white shadow-lg rounded-lg p-8 w-full max-w-md">
      <h2 class="text-2xl font-bold mb-6 text-center text-blue-800">Reset Password (Contact)</h2>
      <form @submit.prevent="onSubmit" class="space-y-4">
        <div>
          <label class="block text-gray-700 mb-1">Email</label>
          <input v-model="form.username_or_email" type="email" required class="w-full px-3 py-2 border rounded focus:outline-none focus:ring-2 focus:ring-blue-400" />
        </div>
        <div>
...
import { resetContactPassword } from '../api/auth';
...
async function onSubmit() {
  ...
  success.value = false;
  try {
    await resetContactPassword(form.value);
    success.value = true;
    setTimeout(() => router.push('/login/contact'), 1500);
  } catch (err) {
