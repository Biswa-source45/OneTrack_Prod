<template>
  <div class="relative">
    ...
    <button
      ...
      class="relative flex items-center justify-center w-10 h-10 rounded-full bg-brand-teal hover:bg-teal-600 focus:outline-none focus:ring-2 focus:ring-teal-300 transition-all duration-200"
      :class="{ 'ring-2 ring-teal-300': isOpen }"
    >
      <!-- Bell Icon -->
      ...
      <span
        ...
        :class="hasUrgent ? 'bg-red-500 animate-pulse' : 'bg-brand-teal'"
      >
        {{ displayCount }}
      </span>
    </button>
    ...
    <div
      v-if="isOpen"
      class="absolute right-0 mt-2 w-80 bg-white rounded-lg shadow-2xl border border-gray-200 z-50 backdrop-blur-sm"
      @click.stop
    >
      <!-- Header -->
      <div class="px-4 py-3 border-b border-gray-200 flex items-center justify-between">
        ...
        <div class="flex items-center gap-2">
          <button
            v-if="unreadCount > 0"
            @click="markAllAsRead"
            class="text-sm text-brand-teal hover:text-teal-700 font-medium"
            :disabled="loading"
          >
            Mark all read
          </button>
      ...
      <div v-else-if="recentNotifications.length > 0" class="max-h-96 overflow-y-auto">
        <div
          ...
          :key="notification.id"
          class="px-4 py-3 border-b border-gray-100 hover:bg-teal-50 cursor-pointer transition-colors"
              ...
              <div class="flex items-start justify-between">
                ...
                <div class="flex items-center gap-1 ml-2">
                  ...
                  <div
                    ...
                    class="w-2 h-2 bg-brand-teal rounded-full animate-pulse"
                  ></div>
                </div>
              </div>
