<template>
  <div class="p-8">
    <PageHeader title="Contact Designations">
      <template #actions>
...
import { fetchContactDesignations } from '../../api/auth';
...
onMounted(load);
async function load(){ rows.value = await fetchContactDesignations(); }
...
async function save(){
  if(openEdit.value){ await api.put(`/designations/contacts/${editingId.value}`, { designation_name: name.value }); }
  else { await api.post(`/designations/contacts`, { designation_name: name.value }); }
  closeForm(); await load();
}
const confirmOpen = ref(false); const selected = ref(null);
function askDelete(row){ selected.value=row; confirmOpen.value=true; }
async function confirmDelete(){ confirmOpen.value=false; if(!selected.value) return; await api.delete(`/designations/contacts/${selected.value.id}`); await load(); }
</script>
