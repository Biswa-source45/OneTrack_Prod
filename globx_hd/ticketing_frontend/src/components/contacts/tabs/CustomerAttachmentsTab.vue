  ...
  <div class="min-h-[600px] flex flex-col">
    ...
    <div class="flex-1 overflow-y-auto p-6">
      ...
      <div v-else-if="!attachments.length" class="text-center py-12">
        ...
        <p class="mt-1 text-sm text-gray-500">Files uploaded to this ticket will appear here.</p>
      </div>
...
const loadAttachments = async () => {
  // Use the full ticket_id from the ticket object, not the route param
  const fullTicketId = props.ticket?.ticket_id || props.ticketId
  if (!fullTicketId) return
  
  loading.value = true
  error.value = ''
  
  try {
    console.log('🔥 DEBUG: Loading attachments for ticket:', fullTicketId)
    const response = await getTicketAttachments(fullTicketId)
